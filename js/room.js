// PeerJS networking for Mythic Survivor. Unlike the sibling games' Host+
// Display room.js, every player here is a full peer that sends actions —
// there's no passive render-only role. See specs/001-p2p-conversion/plan.md
// for the shape of this vs. the Host+Display convention.
//
// This module is a plain transport, not game-aware: it knows nothing about
// `game.js`. The Host side hands incoming { event, payload } pairs to an
// `onMessage` callback and sends back whatever that callback returns as the
// ack; js/main.js is what actually calls into game.js and decides what to
// broadcast.

const ID_PREFIX = 'survivor-room-'; // distinct from the sibling games' prefixes
...
// — this is the direct replacement for Socket.IO's per-emit callback.
...
// unsolicited broadcast from the Host (state, roundResult, chat, emote,
// roomClosed). `onClose(message)` fires when the connection ends.
//
// Resolves to:
